    ...
    .controller('Mantenimiento', function($http, $scope, $stateParams, Camera, Galeria, $cordovaImagePicker, $cordovaToast, $state, $cordovaDialogs, $ionicHistory, $timeout, $ionicLoading, $location, $cordovaGeolocation) {
        ...
        $scope.validateGps = function() {
            if (window.cordova) {
                cordova.plugins.diagnostic.isLocationEnabled(function(enabled) {
                    if (!enabled) {
                        $cordovaDialogs.confirm('Su gps esta desactivado o no ha otorgado permisos.', 'Gps', ['Activar', 'Cancelar'])
                            .then(function(result) {
                                if (result === 1) {
                                    cordova.plugins.diagnostic.switchToLocationSettings();
                                }
        ...
        $cordovaGeolocation.getCurrentPosition(posOptions).then(function(pos) {
            ...
            $cordovaToast.show('No se puede obtener la ubicación, posiblemente el gps este desactivado: ' + error.message, 'short', 'center')
                .then(function(res) {
                    $scope.validateGps();
                });
        });
...
.controller('MapaRuta', function($scope, $http, $cordovaToast, $cordovaDialogs, $ionicLoading, $timeout, $state, $location, $ionicHistory) {
        $scope.cargado = false;
        var gpsnull = true,
            casanull = null,
            marker = null;
        ...
        $scope.calculate = function calculateAndDisplayRoute(directionsService, directionsDisplay) {
            ...
            $http.get($scope.server + '/usuarios/service/list/asignaciones/?asigna=true')
                .then(function doneCallbacks(response) {
                        if (response.data.num_rows === 0) {
